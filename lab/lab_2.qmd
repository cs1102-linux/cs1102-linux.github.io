...
All the necessary files for the lab questions can be found in the `/opt/lab_assets` directory on the course VM
:::


## Combining Commands and Redirections

**Q1:** Check whether the last commands(it could be any command) executes successfully or not, if it executes successfully print `SUCCESS` else `FAIL`. The output should be only `SUCCESS` or `FAIL` in the terminal. **(4 marks)**
...
**Q4:** There are multiple files in the `/lab_assets` directory. Your task is collect all the questions (which are in `.txt` format) and combine them into a single file named `combined_files.txt`. **(4 marks)**
...
**Q7:** ention at least **five** command-line utilities or software tools that you find useful. *(Conduct some research on GitHub and other sources.)*  **(15 marks)**


:::{.callout icon="false"}
Perform this task on your local machine and refer to the instructions.  
...
**Q15:** In continuation to 5th question, now the manager has given file `random_pan_aadhaar.txt` but he is unsure whether it has Aadhaar number or PAN number (case insensitive). So you are required match either PAN or Aadhaar number. **(7 marks)**
...
- A password should have a length of atleast characters
```
...
**Q18:** Log in to the VM and navigate to `/opt/cs1102/lab_assests/NPTEL`. Locate an HTML file with your roll number, copy it to your home directory, and extract only video links from this file using `vi` or `emacs` (Take Screenshot for each operation highlighting bottom of the screen). **(15 marks)**
