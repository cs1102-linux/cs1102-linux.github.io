...
```bash
ssh <CorrectRemoteUserName>@<hoatname>
```
...
::: {.callout-note}
If all the above steps fails, then it is recommended that students generate a new key using the instructions provided in the xourse Website where they initially entered the key to get ssh access.
:::